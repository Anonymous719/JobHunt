...
{
MainWindow conn;
    QString pn = ui->lineEdit_eph->text();
        QString adrs = ui->lineEdit_eadd->text();
        QString name = ui->lineEdit_ename->text();
        QString desc = ui->lineEdit_edesc->text();
        QString id= "1";
    conn.connopen();
...
qry.prepare("UPDATE employerdetials SET CompName='"+name+"',Description='"+desc+"' WHERE id='"+id+"'");
