...
void editprofile::on_pushButton_save_clicked()
{
    MainWindow conn;
        ...
        QSqlQuery qry;
        qry.prepare("UPDATE jobseekerdetials SET Fname='"+fname+"',Lname='"+lname+"',education='"+education+"',currentcompany='"+current+"' WHERE id='"+id+"'");


        //
            if(qry.exec())
            {
                QMessageBox::critical(this,tr("Edit"),tr("FNAME and LNAME Updated Successfully"));
                conn.connclose();
            }
